...
type Config struct {
	...
	// this is an opt-in fuse for unattended/serve setups, not the default
	// posture — ClaudeSessionWarnTokens is.
	ClaudeMaxSessionTokens int `mapstructure:"claude_max_session_tokens"`
	// ClaudeSessionWarnTokens is the replayed-prompt size at which the brain
	// surfaces a visible warning (log + activity feed), once per CLI session.
	// Nothing is dropped: the session keeps resuming. 0 disables the warning.
	ClaudeSessionWarnTokens int `mapstructure:"claude_session_warn_tokens"`
	...
	// SkillsDisabled lists skill names the owner turned off; the loader omits
	// them from the catalog at the single seam every delivery channel (prompt
	// menu, semantic router, read_skill) shares.
...
// envBindings is the only place a config key is bound to an environment
// variable. Explicit bindings only — no AutomaticEnv, which would bind every
// key to its bare upper-cased name and let unrelated vars like the standard
// LANGUAGE leak into (and get persisted over) config values. The settings
// schema reports a key's binding from this same map, so `config schema` can
// never claim a binding the loader does not honor.
var envBindings = map[string]string{
	...
	"skills_similarity_threshold": "SKILLS_SIMILARITY_THRESHOLD",
}
